...
contract Proposal is IProposal, IUpgradableByRequest, PlatformBase, DaoPlatformTypes {
    ...
    *******************/
    ...
    function queue() override public {
        ...
        IVoteEscrowAccount(expectedAccountAddress(proposer)).unlockVoteTokens{
    ...
    function castVote(
        ...
        bool support,
        string reason
    ) override public onlyVoteEscrowAccount(voter) {
        if (state() == ProposalState.Active) {
            ...
            IVoteEscrowAccount(msg.sender).voteCasted{value: 0, flag: MsgFlag.REMAINING_GAS, bounce: false}(id);
        } else {
            IVoteEscrowAccount(msg.sender).rejectVote{value: 0, flag: MsgFlag.REMAINING_GAS, bounce: false}(id);
        }
    }
    ...
    function unlockCastedVote(address accountOwner) override public view onlyVoteEscrowAccount(accountOwner) {
        ProposalState currentState = state();
        bool success = currentState != ProposalState.Active;
        IVoteEscrowAccount(msg.sender).unlockCastedVote{value: 0, flag: MsgFlag.REMAINING_GAS, bounce: false}(id, success);
    }

    function unlockVoteTokens(address accountOwner) override public view onlyVoteEscrowAccount(accountOwner) {
        ProposalState currentState = state();
        bool success = (
        ...
        IVoteEscrowAccount(msg.sender).unlockVoteTokens{value: 0, flag: MsgFlag.REMAINING_GAS, bounce: false}(id, success);
    }

    function expectedAccountAddress(address accountOwner) private view returns (address) {
        return address(tvm.hash(_buildVoteEscrowAccountInitData(VePlatformTypes.VoteEscrowAccount, _buildAccountInitialData(accountOwner))));
    }
    ...
    function _buildVoteEscrowAccountInitData(uint8 platformType, TvmCell initialData) private inline view returns (TvmCell) {
    ...
    function onCodeUpgrade(TvmCell data) private {
        ...
        (voteEscrowRoot, proposer, description, tonActions, ethActions, config, proposalVersion)
